...
| `address/reference-runtime-v0.1/` | Address Schemaの依存なし参照検証器（synthetic / 許可済みデータのみ）。機械可読フィールド型（entities/relations/unknown/lineage sha/minimum_sources/goal.id・success_criteria/state_constraints/capability_scope非空）＋canonical `sort_keys` serialization；`semantic_independence`閉集合enum；`assertion_key`衝突でresidualを埋めない；CLI `--check-contract-only`＋READY/ABSTAIN/CONTRADICTION/STALE/SEMANTIC_INDEPENDENCE_UNMET golden契約fixture（`tools/regenerate_contract_goldens.py`でevaluate再生成）；解決フラグ併用は`INVALID_INPUT` |
