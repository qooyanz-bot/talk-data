...
| `address/reference-runtime-v0.1/` | Address Schemaの依存なし参照検証器（synthetic / 許可済みデータのみ）。`assertion_key`衝突でresidualを埋めない；CLI `--check-contract-only`＋golden契約fixture；解決フラグ併用は`INVALID_INPUT` |
