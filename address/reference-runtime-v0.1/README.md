...
契約goldenの再生成は `python address/reference-runtime-v0.1/tools/regenerate_contract_goldens.py` （固定Address・Evidence・`--now`相当時刻からevaluate()出力を書き戻す）。手編集しない。conformance_report.json の再生成は `python address/reference-runtime-v0.1/tools/regenerate_conformance_report.py`。
