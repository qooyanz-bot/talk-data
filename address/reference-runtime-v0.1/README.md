...
- Conformance runner（`conformance.py` / CLI `--conformance`）。`run_conformance() -> dict` が機械可読レポートを返す（`schema_version=address-conformance-v1`、`checks: [{id, status, detail}]`、overall=`CONFORMANT`|`FAIL`）。集約内容: (1) LIMITATIONS 文書を status=`LIMITATIONS` で埋め込み（PASS 能力として再解釈しない）、(2) synthetic battery: `valid_synthetic_address.json` validate→VALID；evaluate READY 経路 value=null；shared-law ABSTAIN 経路 value=null、(3) `r6g_frozen_protocol_manifest.json` validate→MANIFEST_VALID；R6-G EXPERIMENT_RESULT 主張は BLOCKED / NOT_RUN（レポートに NOT_RUN|BLOCKED、実行済みと主張しない）。exit 0=全必須チェック成功（overall CONFORMANT）、いずれか FAIL なら非0。他 standalone モードと相互排他。
