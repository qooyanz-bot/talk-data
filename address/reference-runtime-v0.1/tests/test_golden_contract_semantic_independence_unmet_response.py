"""Freeze a saved public SEMANTIC_INDEPENDENCE_UNMET ABSTAIN response for --check-contract-only."""
...
class GoldenContractSemanticIndependenceUnmetResponseTests(unittest.TestCase):
    def setUp(self):
        self.fixture_path = ROOT / "fixtures" / "golden_contract_semantic_independence_unmet_response.json"
    ...
    def test_fixture_passes_response_contract_validate(self):
        ...
        self.assertEqual(self.response["resolution"]["reason"], "SEMANTIC_INDEPENDENCE_UNMET")
        ...
        self.assertEqual(self.response["generated_audit"]["reason"], "SEMANTIC_INDEPENDENCE_UNMET")
        details = self.response["resolution"]["details"]
        self.assertTrue(any("AUDITED" in str(item) for item in details))
        self.assertTrue(any("CONTRACTED" in str(item) for item in details))
    ...
    def test_fixture_decision_must_remain_abstain_semantic_independence_unmet(self):
        """Regression: must not silently become READY or claim audited independence."""
        self.assertEqual(self.response["resolution"]["decision"], "ABSTAIN")
        self.assertEqual(self.response["resolution"]["reason"], "SEMANTIC_INDEPENDENCE_UNMET")
        self.assertEqual(self.response["generated_audit"]["decision"], "ABSTAIN")
        self.assertEqual(self.response["generated_audit"]["reason"], "SEMANTIC_INDEPENDENCE_UNMET")
        ...
        self.assertTrue(
            ...
            or broken["resolution"]["reason"] != "SEMANTIC_INDEPENDENCE_UNMET"
        )
        self.assertNotEqual(broken["resolution"]["reason"], "SEMANTIC_INDEPENDENCE_UNMET")
