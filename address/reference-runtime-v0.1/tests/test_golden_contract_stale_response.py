"""Freeze a saved public EVIDENCE_STALE ABSTAIN response fixture for --check-contract-only."""
...
class GoldenContractStaleResponseTests(unittest.TestCase):
    def setUp(self):
        self.fixture_path = ROOT / "fixtures" / "golden_contract_stale_response.json"
    ...
    def test_fixture_passes_response_contract_validate(self):
        ...
        self.assertEqual(self.response["resolution"]["reason"], "EVIDENCE_STALE")
        ...
        self.assertEqual(self.response["generated_audit"]["reason"], "EVIDENCE_STALE")
        # Stale evidence_ids listed in details (observed_at past freshness max_age vs --now).
        self.assertIn("e-1", self.response["resolution"]["details"])
        self.assertIn("e-2", self.response["resolution"]["details"])
    ...
    def test_fixture_decision_must_remain_abstain_evidence_stale(self):
        """Regression: golden EVIDENCE_STALE path must not silently become READY or other reasons."""
        self.assertEqual(self.response["resolution"]["decision"], "ABSTAIN")
        self.assertEqual(self.response["resolution"]["reason"], "EVIDENCE_STALE")
        self.assertEqual(self.response["generated_audit"]["decision"], "ABSTAIN")
        self.assertEqual(self.response["generated_audit"]["reason"], "EVIDENCE_STALE")
        ...
        self.assertTrue(
            ...
            or broken["resolution"]["reason"] != "EVIDENCE_STALE"
        )
        self.assertNotEqual(broken["resolution"]["reason"], "EVIDENCE_STALE")
