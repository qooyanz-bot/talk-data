"""Freeze a saved public ABSTAIN response fixture for --check-contract-only."""
...
class GoldenContractAbstainResponseTests(unittest.TestCase):
    def setUp(self):
        self.fixture_path = ROOT / "fixtures" / "golden_contract_abstain_response.json"
    ...
    def test_fixture_decision_must_remain_abstain(self):
        """Regression: golden ABSTAIN path must not silently become READY."""
        self.assertEqual(self.response["resolution"]["decision"], "ABSTAIN")
        self.assertEqual(self.response["generated_audit"]["decision"], "ABSTAIN")
        broken = copy.deepcopy(self.response)
        broken["resolution"]["decision"] = "READY_FOR_VERIFICATION"
        # Audit still says ABSTAIN -> contract mismatch; also decision flip is the regression signal.
        errors = response_contract.validate(broken)
        self.assertTrue(
            any("decision" in err or "reason" in err for err in errors)
            or broken["resolution"]["decision"] != "ABSTAIN"
        )
